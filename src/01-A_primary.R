...
students_course_primary_core <- students_local_course_id_title_section_number %>%
  ...
  left_join(local_number_isbe_state_course_ids, by = c("grade_level", "school")) %>% 
...
students_course_primary_excellence <- students_local_course_id_title_section_number %>%
  filter(grepl("k |1|2|3", local_course_title),
         !grepl("Science", local_course_title)) %>%
  mutate(school = str_extract(local_course_id, "kop|kbp|kap|kacp"),
         grade_level = str_replace(local_course_id, "kop|kbp|kap|kacp", "") %>%      
